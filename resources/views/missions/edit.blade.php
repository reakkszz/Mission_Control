            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <form action="{{ route('missions.update', $mission)}}" method="POST">
                        ...
                        >
                            ...
                            @foreach ($operations as $operation)
                                <option value="{{ $operation->id }}"
                                    {{ old('operation_id', $mission->operation_id) == $operation->id ? 'selected' : '' }}>
                                    {{ $operation->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-6">
                        ...
                        >
                            <option value="todo"
                            {{ old('status', $mission->status) === 'todo' ? 'selected' : '' }}>
                                Todo
                            </option>
                            <option value="in_progress" {{ old('status',$mission->status) === 'in_progress' ? 'selected' : '' }}>
                                In Progress
                            </option>
                            <option value="completed" {{ old('status',$mission->status) === 'completed' ? 'selected' : '' }}>
                                Completed
                            </option>
                        </select>
